...
import proyectoanalista.negocio.IProveedorManager;
import proyectoanalista.negocio.IRepuestoManager;
import proyectoanalista.negocio.ProveedorManager;
...
public class AgregarRepuesto extends JDialog {
	...
	public AgregarRepuesto( JFrame padre, String titulo, String titleBorder, DetallarServicio detallarServicio, Servicio servicio) {
		...
		jPanelAgregarRepuesto.setLayout( new FlowLayout(FlowLayout.RIGHT, 0,10) );
	...
	private void inicializarVariables() {
		...
		jTextFieldPrecioUnitario.addKeyListener(new java.awt.event.KeyAdapter() {  
            // selecciona el boton aceptar cuando se presiona Enter en el jTextFieldPrecioUnitario, para que parsee de double a string
		...
		jButtonAceptar.addActionListener( new JButtonActionListener( aceptarAgregarRepuesto ) );
		jButtonAceptar.addKeyListener( new JButtonKeyListener( aceptarAgregarRepuesto ) );
		...
		jButtonCancelar.addActionListener( new JButtonActionListener( cancelarAgregarRepuesto ) );
		jButtonCancelar.addKeyListener( new JButtonKeyListener( cancelarAgregarRepuesto ) );
	...
	private void aceptarAgregarRepuesto() {
		...
		if( validarDatos() && characterVaryingExceeded() ) {
			...
			double precio_unitario; // almacena el precio unitario del repuesto una vez presionado el boton Aceptar
			double precio_total; // almacena el precio total del repuesto/s una vez presionado el boton Aceptar
			precio_unitario = ((Number)jTextFieldPrecioUnitario.getValue()).doubleValue();
			precio_total = precio_unitario * (Integer.parseInt(jTextFieldCantidad.getText()));
			repuesto.setNombre( jTextFieldNombre.getText().trim() );
			repuesto.setPrecioUnitario( precio_total );
			...
			switch( option ) {
				case 1:
					...
					Object fila1[] = {Integer.toString(repuesto.getId_repuesto()), repuesto.getNombre(), jComboBoxProveedor.getSelectedIndex() == 0 ? "< sin especificar >" : (listaProveedores.get(jComboBoxProveedor.getSelectedIndex() - 1).getNombre() ), amountDisplayFormat.format(precio_unitario), (jTextFieldCantidad.getText().trim()), (jTextAreaObservaciones.getText().trim().equals("") ? "" : "\u2713"), amountDisplayFormat.format(precio_total), Integer.toString(detallarServicio.dtmRepuestos.getRowCount()) };
	...
	private void cerrarEsc(){
		...
		jPanelAgregarRepuesto.registerKeyboardAction(actionListener, keystroke, JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
	}
