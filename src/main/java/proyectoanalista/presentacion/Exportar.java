...
import proyectoanalista.negocio.IMecanicoManager;
...
import proyectoanalista.negocio.MecanicoManager;
...
//		fileChooser.showOpenDialog(padre); // setea icono al fileChooser
	...
	String sentenciaClientes(){
		...
		if(!listaClientes.isEmpty())
			stringBuilder.append("INSERT INTO cliente (id_cliente,cuit,fecha_nacimiento,nombre,apellido,telefono,celular,domicilio,localidad,codigo_postal,mail,observaciones) VALUES\n");
		
		while (iterator.hasNext()){
			cliente = iterator.next();
			stringBuilder.append("('" + cliente.getId_cliente() + "', '" + cliente.getCUIT() + "', '" + new java.sql.Date(	cliente.getFechaDeNacimiento().getTimeInMillis()) + "', '" + cliente.getNombre().trim().replace("'", "''") + "', '" + cliente.getApellido().trim().replace("'", "''") + "', '" + cliente.getTelefono().trim() + "', '" + cliente.getCelular() + "', '" + cliente.getDomicilio().replace("'", "''") + "', '" + cliente.getLocalidad().replace("'", "''") + "', '" + cliente.getCodigoPostal().replace("'", "''") + "', '" + cliente.getEmail().trim() + "', '" + cliente.getObservaciones().replace("'", "''") + "')");
			if(iterator.hasNext())
