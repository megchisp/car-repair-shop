	...
	public int agregar(Cliente cliente) throws Exception {
		try{
			if (esValidoParaAgregar(cliente)){ 
				String query = "INSERT INTO cliente (id_cliente, cuit,fecha_nacimiento,nombre,apellido,telefono,celular,domicilio,localidad,codigo_postal,mail,observaciones) VALUES ('"+ cliente.getId_cliente() + "','"+ cliente.getCUIT() + "','" + new java.sql.Date(cliente.getFechaDeNacimiento().getTimeInMillis()) + "','" + cliente.getNombre().replace("'", "''") + "','" + cliente.getApellido().replace("'", "''") + "','" + cliente.getTelefono() + "','" + cliente.getCelular() + "','" + cliente.getDomicilio().replace("'", "''") + "','" + cliente.getLocalidad().replace("'", "''") + "','" + cliente.getCodigoPostal().replace("'", "''") + "','" + cliente.getEmail() + "','" + cliente.getObservaciones().replace("'", "''") + "');";
